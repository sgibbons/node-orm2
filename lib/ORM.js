...
ORM.prototype.define = function (name, properties, opts, callback) {
  ...
  if(opts.infer) {
    ...
    var original_scope = this;
    properties = this.driver.infer(opts.table || inflection.tableize(name), function(err, properties) {
      if(err) {
        throw('Error loading properties!'); // TODO: actually pass back along callback chain
      } else {
        buildModel(properties, original_scope.settings, original_scope.driver, original_scope.models, callback)
      }
    });
  } else {
